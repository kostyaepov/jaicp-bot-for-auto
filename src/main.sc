start
    ...
    + (Найди|Подбери|Покажи) {марка:word}
        $session.make = "{марка}"
        -> search_state

    + (Найди|Подбери|Покажи) {марка:word} {модель:word}
        $session.make = "{марка}"
        $session.model = "{модель}"
        -> search_state

    + (Найди|Подбери|Покажи) {марка:word} до {цена:regex(\\d+)} рублей
        $session.make = "{марка}"
        $session.max_price = {цена}
        -> search_state

    + (Найди|Покажи|Помоги найти) {марка:word} {модель:word} до {цена:regex(\\d+)} рублей с пробегом до {пробег:regex(\\d+)} км
        $session.make = "{марка}"
        $session.model = "{модель}"
        $session.max_price = {цена}
